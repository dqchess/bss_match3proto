...
namespace GameVanilla.Game.Common
{
    ...
    public class GameBoard : MonoBehaviour
    {
            ...
            if (drag && selectedTile != null)
            {
                ...
                if (hit.collider != null && hit.collider.gameObject != selectedTile)
                {
                    ...
                    if (combo != null)
                    {
                        ...
                        // combo x combo
                        LeanTween.move(selectedTile, hit.collider.gameObject.transform.position, PuzzleMatchManager.instance.tweenConstant.tileMoveTime/*0.25f*/).setOnComplete(
                            () =>
                            {
                        ...
                        LeanTween.move(hit.collider.gameObject, selectedTile.transform.position, PuzzleMatchManager.instance.tweenConstant.tileMoveTime); // 0.25f);
                    ...
                    else
                    {
                        ...
                        {
                            currentlySwapping = true;
                            LeanTween.move(selectedTile, hitTilePos, PuzzleMatchManager.instance.tweenConstant.tileMoveTime); // 0.2f);
                            LeanTween.move(hit.collider.gameObject, selectedTilePos, PuzzleMatchManager.instance.tweenConstant.tileMoveTime/*0.2f*/).setOnComplete(() =>
                            {
                                // move 되돌아오기
                                LeanTween.move(selectedTileCopy, selectedTilePos, PuzzleMatchManager.instance.tweenConstant.tileMoveTime/*0.2f*/).setOnComplete(() =>
                                {
                                    currentlySwapping = false;
                                    selectedTileCopy.GetComponent<SpriteRenderer>().sortingOrder = 0;
                            ...
                            LeanTween.move(hitTileCopy, hitTilePos, PuzzleMatchManager.instance.tweenConstant.tileMoveTime);// 0.2f);
                            });
                        }
        ...
        private IEnumerator ExplodeGeneratedTilesAsync(List<GameObject> genTiles)
        {
            ...
            StartCoroutine(ApplyGravityAsync(PuzzleMatchManager.instance.tweenConstant.specialExplodeDelay1)); // 0.5f));
        }
        ...
        private bool HandleMatches(bool isPlayerMatch)
        {
            ...
            if (matches.Count > 0)
            {
                ...
                StartCoroutine(ApplyGravityAsync(didAnySpecialCandyExplode ? PuzzleMatchManager.instance.tweenConstant.specialExplodeDelay2 /*0.5f*/ : 0.0f));
                return true;
            }
            else
            {
        ...
        private IEnumerator ApplyGravityAsync(float delay = 0.0f)
        {
            ...
            possibleSwaps = DetectPossibleSwaps();
            yield return new WaitForSeconds(PuzzleMatchManager.instance.tweenConstant.afterDropDelay); // 1.0f);
            if (currentlyAwarding)
            {
                gameScene.CheckEndGame();
            }
                    ...
                    if (bottom != -1)
                    {
                        ...
                        if (tile != null)
                        {
                            ...
                            var tween = LeanTween.move(tile,
                                ...
                                PuzzleMatchManager.instance.tweenConstant.tileDropTime);
                                //0.5f);
                ...
                if (numEmpties > 0)
                {
                    for (var j = 0; j < level.height; j++)
                    {
                        ...
                        if (tiles[tileIndex] == null && !isHole)
                        {
                            ...
                            var tween = LeanTween.move(tile,
                                targetPos,
                                PuzzleMatchManager.instance.tweenConstant.tileDropTime); // 0.5f);
                            tween.setEase(LeanTweenType.easeInQuad);
                            tween.setOnComplete(() =>
                            {
                                if (tile.activeSelf && tile.GetComponent<Animator>() != null)
